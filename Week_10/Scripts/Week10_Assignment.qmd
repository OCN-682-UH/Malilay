...
```{r}
filterby_coral <- function(data, coralcov_min, coralcov_max) { #choose
  filtered_data <- data %>%
    filter(coral_cover >= coralcov_min,
           coral_cover <= coralcov_max)
  return(filtered_data)
}
...
```{r}
structuralcomplexityplot <- function(data, coralcov_min, coralcov_max) {
  filtered_data <- filterby_coral(data = data, 
                                  coralcov_min = coralcov_min, 
                                  coralcov_max = coralcov_max) %>%
    mutate(light_availability = factor(light_availability, levels = c("Low", "Medium", "High")))
