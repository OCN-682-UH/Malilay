...
```{r}
mpastatus_to_coralcover <- function(data, mpa_status) { #name the function and arguments
  if (mpa_status == "MPA") { #if then logic if mpa status is an MPA
    meanvalue <- data %>% #assign mean value
      group_by(region) %>% #group by the four regions
      summarise(meanval = mean(coral_cover, na.rm = TRUE)) #get mean value of coral cover across region
  } else { #when mpa status is NOT MPA (which means it is unprotected)
    meanvalue <- data %>% #assign mean value and pipe data
      group_by(region) %>%
      summarise(meanval = mean(algal_cover, na.rm = TRUE)) #algal cover if NOT MPA
  }
  return(meanvalue) #output is mean value of coral cover or algal cover
}
#Test
...
#Let's test!
 structuralcomplexityplot(data = coral, coralcov_min = 70, coralcov_max = 90)
```
Take a look across the entire range of coral cover values
```{r}
structuralcomplexityplot(data = coral, coralcov_min = 0, coralcov_max = 100)
```
